...
[x] setting content size of a stack container within a dsg flex row works on the front end but it stays it's normal width in the editor
[x] Blobs are stuck align left in the editor even after setting alignment. It's fine on the front end.
[x]When pasting a block while a container is focused on, it should add it to the end of the containter, within it. Current state, it replaces it no matter what.
