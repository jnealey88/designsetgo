...
export default function MapSettingsPanel({ attributes, setAttributes }) {
	...
	const handleAddressSearch = useCallback(async () => {
		if (!dsgoAddress || dsgoAddress.trim() === '') {
			setSearchError(__('Please enter an address to search.', 'designsetgo'));
			return;
		}
		...
		try {
			...
			if (result) {
				...
				setSearchError(__('Address not found. Please try a different search.', 'designsetgo'));
			}
		} catch (error) {
			setSearchError(__('Failed to search address. Please try again.', 'designsetgo'));
		} finally {
			setIsSearching(false);
		}
	}, [dsgoAddress, setAttributes]);

	// Create debounced version that waits 1 second (respects Nominatim rate limits)
	...
	return (
		<PanelBody title={__('Map Settings', 'designsetgo')} initialOpen={true}>
			...
			<SelectControl
				...
				value={dsgoProvider}
				options={[
					{ label: __('OpenStreetMap (No API key required)', 'designsetgo'), value: 'openstreetmap' },
					{ label: __('Google Maps (Requires API key)', 'designsetgo'), value: 'googlemaps' },
				]}
			...
			{dsgoProvider === 'googlemaps' && (
				<>
					{window.dsgoIntegrations?.googleMapsApiKey ? (
						<Notice status="success" isDismissible={false} style={{ marginTop: '12px' }}>
							{__('✓ Google Maps API key configured in ', 'designsetgo')}
							<a href="/wp-admin/admin.php?page=designsetgo-settings" target="_blank" rel="noopener noreferrer">
						...
						<Notice status="warning" isDismissible={false} style={{ marginTop: '12px' }}>
							<strong>{__('⚠ No API key configured.', 'designsetgo')}</strong>{' '}
							...
							<br />
							<small>
								<strong>{__('Security:', 'designsetgo')}</strong>{' '}
								{__('Configure HTTP referrer restrictions in Google Cloud Console.', 'designsetgo')}
							</small>
						</Notice>
					)}
				</>
			...
			<div style={{ marginTop: '24px' }}>
				...
				<TextControl
					...
					placeholder={__('Enter an address or location', 'designsetgo')}
					help={__('Search for a location to automatically set coordinates.', 'designsetgo')}
					__next40pxDefaultSize
					__nextHasNoMarginBottom
				/>
				...
				<Button
					...
					style={{ marginTop: '8px' }}
				>
					{isSearching ? __('Searching...', 'designsetgo') : __('Search Address', 'designsetgo')}
				</Button>
				...
				<div style={{ marginTop: '16px' }}>
					<TextControl
						...
						min="-90"
						max="90"
						help={__('Manual coordinate entry (between -90 and 90).', 'designsetgo')}
						__next40pxDefaultSize
						__nextHasNoMarginBottom
					/>
					...
					<TextControl
						...
						min="-180"
						max="180"
						help={__('Manual coordinate entry (between -180 and 180).', 'designsetgo')}
						__next40pxDefaultSize
						__nextHasNoMarginBottom
					/>
				</div>
				...
				<RangeControl
					...
					max={20}
					step={1}
					help={__('1 = world view, 20 = street level.', 'designsetgo')}
					__next40pxDefaultSize
					__nextHasNoMarginBottom
				/>
			</div>
			...
			<div style={{ marginTop: '24px' }}>
				...
				<TextControl
					...
					onChange={(value) => setAttributes({ dsgoMarkerIcon: value || '📍' })}
					help={__('Enter an emoji or icon character.', 'designsetgo')}
					__next40pxDefaultSize
					__nextHasNoMarginBottom
				/>
			</div>
			...
			<div style={{ marginTop: '24px' }}>
				...
				<SelectControl
					...
					options={[
						{ label: __('16:9 (Widescreen)', 'designsetgo'), value: '16:9' },
						{ label: __('4:3 (Standard)', 'designsetgo'), value: '4:3' },
						{ label: __('1:1 (Square)', 'designsetgo'), value: '1:1' },
						{ label: __('Custom Height', 'designsetgo'), value: 'custom' },
					]}
					onChange={(value) => setAttributes({ dsgoAspectRatio: value })}
					help={
						dsgoAspectRatio === 'custom'
							? __('Set a custom height below.', 'designsetgo')
							: __('Maintains aspect ratio across screen sizes.', 'designsetgo')
					}
					__next40pxDefaultSize
					__nextHasNoMarginBottom
				/>
				...
				{dsgoAspectRatio === 'custom' && (
					<UnitControl
						...
						value={dsgoHeight}
						onChange={(value) => setAttributes({ dsgoHeight: value || '400px' })}
						units={[
							{ value: 'px', label: 'px' },
							{ value: '%', label: '%' },
							{ value: 'vh', label: 'vh' },
						]}
						help={__('Set a custom height for the map.', 'designsetgo')}
						__next40pxDefaultSize
						__nextHasNoMarginBottom
					/>
				)}
				...
				{dsgoProvider === 'googlemaps' && (
					<SelectControl
						...
						value={dsgoMapStyle}
						options={[
							{ label: __('Standard', 'designsetgo'), value: 'standard' },
							{ label: __('Silver (Minimalist)', 'designsetgo'), value: 'silver' },
							{ label: __('Dark Mode', 'designsetgo'), value: 'dark' },
						]}
						onChange={(value) => setAttributes({ dsgoMapStyle: value })}
						help={__('Choose a visual style for Google Maps.', 'designsetgo')}
						__next40pxDefaultSize
						__nextHasNoMarginBottom
					/>
				)}
			...
			<div style={{ marginTop: '24px' }}>
				...
				<ToggleControl
					...
					onChange={(value) => setAttributes({ dsgoPrivacyMode: value })}
					help={
						dsgoPrivacyMode
							? __('Map will not load until user clicks to consent.', 'designsetgo')
							: __('Map will load automatically when page loads.', 'designsetgo')
					}
					__nextHasNoMarginBottom
				/>
				...
				{dsgoPrivacyMode && (
					<>
						<TextareaControl
							...
							onChange={(value) => setAttributes({ dsgoPrivacyNotice: value })}
							rows={4}
							help={__('Message shown to users before loading the map.', 'designsetgo')}
							__nextHasNoMarginBottom
						/>
